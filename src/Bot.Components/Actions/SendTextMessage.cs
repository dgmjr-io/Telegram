...
public class SendTextMessage(
    ...
    : TelegramBotCustomAction<SendTextMessage>(
        telemetryClient,
    ...
    )
{
    public new const string DeclarativeTypeConst =
        $"{Constants.Namespace}.{nameof(SendTextMessage)}";
        ...
        var message = await CallBotAsync(
            ...
            bot =>
                bot.SendTextMessageAsync(
                    chatId: GetChatId(dc),
                    text: ParseMode.GetValue(dc.State)
                        is ParseModeEnum.Markdown
                            or ParseModeEnum.MarkdownV2
                        ? Message.GetValue(dc.State).EscapeMarkdown()
                        : Message.GetValue(dc.State),
